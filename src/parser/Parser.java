...
import fichiers.ParserManquantException;
...
public abstract class Parser {
	...
	/**
	 * La fonction traiter() parcours la liste à la recherche d'un maillon qui sait
	 * comment parser la ligne. Dans ce cas la ligne est parsée et la recherche
	 * s'arrête
	 * 
	...
	public void traiter(String ligne) throws Exception {
		if (saitParser(ligne))
			...
			// S'il ne sait pas mais qu'il a un suivant dans la liste chaine, il lui repasse
			// la ligne et qu'il se débrouille !
			getSuivant().traiter(ligne);
		else
			// Sinon, on est arrivé au bout sans trouver un parser
			// et on lance une exception ! Que le prog appelant se débrouille avec cette
			// ligne !
	...
	/**
	 * Renvoie true si le parser en question reconnait le type de ligne,
	 * c'est-à-dire qu'il sait la "décortiquer", et créer le ou les objets qu'il
	 * faut. Il n'y a pas d'exception. En cas de problème, on renvoie false !
	 * 
	 * @param ligne
